...
cd ..
cd ansible
ansible-playbook deploy.yml -e "memory=$MEMORY" -e "cpus=$CPUS" -e "instances=$INSTANCES" -e "app_group=$APP_GROUP" -e "app_id=$APP_ID" -e "tag=$TAG" -e "container_port=$CONTAINER_PORT" -e "service_port=$SERVICE_PORT" -e "slavetype=$SLAVETYPE"
